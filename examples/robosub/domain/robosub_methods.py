...
def tm1_move(state, l, l_):
    if state.loc['r'] == l:
        yield [('a_search_for', l_), ('a_move', l, l_)]
...
def tm1_cross_gate(state, g, l):
    if state.loc['r'] == l and state.loc[g] == l:
        yield [('a_localize', g, l), ('a_cross_gate_40', g, l)]


def tm2_cross_gate(state, g, l):
    if state.loc['r'] == l and state.loc[g] == l:
        yield [('a_localize', g, l), ('a_cross_gate_60', g, l)]
...
def tm1_pick(state, b, l):
    if state.loc['r'] == l and state.loc[b] == l:
        yield [('a_localize', b, l), ('a_pick', b, l)]
...
def tm1_trace_path(state, gp, l):
    if state.loc['r'] == l and state.loc[gp] == l:
        yield [('a_localize', gp, l), ('a_trace_guide_path', gp, l)]
...
def tm1_slay_vampire(state, v, l):
    if state.loc['r'] == l and state.loc[v] == l:
        yield [('a_localize', v, l), ('a_touch_back_v', v, l)]


def tm2_slay_vampire(state, v, l):
    if state.loc['r'] == l and state.loc[v] == l:
        yield [('a_localize', v, l), ('a_touch_front_v', v, l)]
...
def tm1_loc3(state):
    if state.loc['r'] == 'l3':
        yield [('drop_garlic_task', 'gm1', 'c1', 'l3'),
                ('pick_task', 'gm2', 'l3'),
...
def tm1_drop_garlic(state, gm, c, l):
    if state.loc['r'] == l and state.loc[c] == l:
        yield [('a_localize', c, l), ('a_open_c', c, l), ('a_drop_garlic_closed_coffin', gm, c, l)]


def tm2_drop_garlic(state, gm, c, l):
    if state.loc['r'] == l and state.loc[c] == l:
        yield [('a_localize', c, l), ('a_drop_garlic_open_coffin', gm, c, l)]
...
def tm1_loc4(state):
    if state.loc['r'] == 'l4':
        yield [('stake_through_heart_task', 'd1', 'l4'),
...
def tm1_loc5(state):
    if state.loc['r'] == 'l5':
        yield [('a_localize', 's1', 'l5'), ('a_surface', 'cm1', 's1', 'l5')]


def tm2_loc5(state):
    if state.loc['r'] == 'l5':
        yield [('pick_task', 'cm2', 'l5'), ('o_localize', 's1', 'l5'), ('o_surface', 'cm2', 's1', 'l5')]
